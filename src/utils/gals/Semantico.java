...
private void acao7(Token token) throws SemanticError {
    ...
    if (!"int64".equals(tipo) && !"float64".equals(tipo)) {
        throw new SemanticError(
            "tipo incompativel em operador aritmético +",
            token.getPosition()
        );
    }
	...
	private void acao18(Token token) {
		...
		char c = token.getLexeme().charAt(1);
		int valor;
		switch (c) {
			case 'n': valor = 10; break;
			case 't': valor = 9; break;
			case 's': valor = 32; break;
	...
	private void acao28() {
		...
		String falso = pilhaRotulos.pop();
		String fim = pilhaRotulos.pop();

		codigoObjeto.append("br " + fim + "\n");

		pilhaRotulos.push(fim);
		codigoObjeto.append(falso + ":\n");
	}
	...
	private void acao33() {
	    ...
	    String inicio = pilhaRotulos.pop();
